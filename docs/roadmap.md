...
| Cloudflare's analytics beacon is blocked by the CSP                                   | `script-src 'self' 'wasm-unsafe-eval'` admits no third-party script, and Cloudflare injects `static.cloudflareinsights.com/beacon.min.js` into the live site — so it is refused on every load and the console carries the violation. Nothing in the app depends on it. Left blocked deliberately rather than widening the policy: the editor renders HTML pasted from Word and Docs, which is the reason the policy is that tight. The alternatives are turning RUM injection off at Cloudflare, or allowing that one host in `script-src` plus `cloudflareinsights.com` in `connect-src`.                                                                           |
